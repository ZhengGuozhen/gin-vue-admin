...
export default {
  ...
  computed: {
    showSelfNode() {
      if (
        this.wf.assignType == "self" &&
        this.move.promoterID == this.userInfo.ID
      ) {
        return true;
      } else {
        return false;
    ...
    canShow() {
      if (this.wf.assignType == "user") {
        if (
          this.wf.assignValue.indexOf("," + this.userInfo.ID + ",") > -1 &&
          this.wf.clazz == "userTask"
        ) {
          return true;
        } else {
          return false;
        ...
        if (
          this.wf.assignValue.indexOf("," + this.userInfo.authorityId + ",") >
            -1 &&
          this.wf.clazz == "userTask"
        ) {
          return true;
    ...
    async start() {
      const res = await startWorkflow({
        ...
        wf: {
          ...
          businessId: 0,
          businessType: "demoCodeGeneration",
